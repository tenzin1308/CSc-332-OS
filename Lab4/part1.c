...
Author: Tenzin Tashi
Task 4, part 1: Write a special simple command interpreter that takes
	command and its arguments. This interpreter is a program where the main 
...
int main(int argc, char *argv[])
{
    ...
    while (strcmp(c, "quit"))
    { // terminated when enters quit

        token = strtok(c, " "); /* get the first token */
        int i = 0;
        while (token != NULL)
        {
